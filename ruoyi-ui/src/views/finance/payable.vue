  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch">
      <el-form-item label="应付单号" prop="payableNo">
        <el-input v-model="queryParams.payableNo" placeholder="请输入应付单号" clearable style="width: 220px" @keyup.enter="handleQuery" />
      </el-form-item>
    ...
    <el-table v-loading="loading" :data="dataList">
      <el-table-column label="应付单号" prop="payableNo" align="center" min-width="170" />
      ...
      <el-table-column label="应付金额" prop="billAmount" align="right" />
      ...
      <el-table-column label="操作" align="center" width="120" fixed="right">
        <template #default="scope">
          <el-button link type="primary" @click="handleWriteoff(scope.row)" v-hasPermi="['finance:payable:writeoff']">核销</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog title="应付核销" v-model="open" width="520px" append-to-body>
      <el-form ref="writeoffRef" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="应付单号">
          <el-input v-model="form.payableNo" disabled />
        </el-form-item>
        <el-form-item label="流水号" prop="cashFlowNo">
          <el-input v-model="form.cashFlowNo" placeholder="请输入已付款流水号" />
        </el-form-item>
...
import { listPayable, writeoffPayable } from '@/api/finance/payable'
...
const data = reactive({
  form: {},
  queryParams: { pageNum: 1, pageSize: 10, payableNo: undefined, periodCode: undefined, counterpartyName: undefined },
  rules: {
...
function getList() {
  loading.value = true
  listPayable(queryParams.value).then(res => { dataList.value = res.rows; total.value = res.total }).finally(() => { loading.value = false })
}
function handleQuery() { queryParams.value.pageNum = 1; getList() }
function resetQuery() { proxy.resetForm('queryRef'); handleQuery() }
function handleWriteoff(row) {
  form.value = { payableNo: row.payableNo, cashFlowNo: undefined, writeoffAmount: Number(row.remainAmount || 0), remark: undefined }
  open.value = true
}
function submitWriteoff() {
  proxy.$refs.writeoffRef.validate(valid => {
    if (!valid) return
    writeoffPayable(form.value.payableNo, form.value).then(() => {
