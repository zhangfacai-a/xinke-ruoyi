    ...
    for attempt in range(RETRIES + 1):
        try:
            ...
            if result.get("code") != 200:
                raise RuntimeError("心跳失败: code={0}, msg={1}".format(result.get("code"), result.get("msg") or "未知错误"))
            return result.get("data") or {}
        except error.HTTPError as exc:
            detail = exc.read().decode("utf-8", errors="replace")
            if exc.code < 500 or attempt >= RETRIES:
                raise RuntimeError("心跳请求失败: HTTP {0}, {1}".format(exc.code, detail[:500]))
        ...
        print("心跳暂时不可用，{0}秒后重试".format(wait_seconds))
        sleep(wait_seconds)
...
def main(args):
    ...
    print("续租批次: " + payload["batchNo"])
    data = _post_json(BASE_URL.rstrip("/") + "/batch/heartbeat", payload)
    print("批次续租成功，当前任务数: {0}".format(data.get("activeTaskCount", 0)))
    return data
