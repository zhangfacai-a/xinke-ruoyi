  ...
  <div :class="['gift-catalog-page', { 'app-container': !embedded }]">
    ...
    <div v-loading="loading" class="gift-grid">
      <article v-for="row in rows" :key="row.giftId" :class="['gift-card',{disabled:row.status!=='0','personal-hidden':row.personalHidden}]">
        <div class="gift-card-head"><div class="gift-title"><strong>{{ row.giftName }}</strong><span class="code">{{ row.giftCode }}</span></div><div class="status-control"><span>{{ row.status==='0'?'启用':'停用' }}</span><el-switch v-model="row.status" active-value="0" inactive-value="1" :loading="row.statusSaving" @change="toggleStatus(row)" /></div></div>
        <div class="gift-metrics"><div><small>当前成本</small><strong v-if="row.currentCost!=null" class="cost">¥{{ money(row.currentCost) }}</strong><el-tag v-else type="danger" effect="plain">未设置</el-tag></div><div><small>可用库存</small><strong :class="stockClass(row)">{{ inventoryMap[row.giftId]?.stockQty ?? 0 }}<em>件</em></strong><span :class="['stock-state',stockClass(row)]">{{ stockLabel(row) }}<template v-if="inventoryMap[row.giftId]?.safetyQty"> · 安全值 {{ inventoryMap[row.giftId].safetyQty }}</template></span></div></div>
        <div class="alias-line" :title="row.aliases"><span>搜索别名</span><p>{{ row.aliases || '暂无别名' }}</p></div>
        <div class="gift-card-actions"><el-button link icon="Clock" @click="openHistory(row)">变更记录</el-button><span class="action-spacer" /><el-button link type="primary" icon="Edit" @click="openGift(row)">编辑</el-button><el-button v-hasPermi="['live:gift:cost']" link type="warning" icon="Money" @click="openCost(row)">调成本</el-button></div>
      </article>
...
import { addGiftCost, getGift, importGiftCosts, listGiftInventory, listGifts, saveGift } from '@/api/live/gift'
const props = defineProps({ embedded: { type: Boolean, default: false } })
const { proxy }=getCurrentInstance()
const today=()=>new Date().toISOString().slice(0,10)
const statusOptions=[{label:'全部',value:''},{label:'启用',value:'0'},{label:'停用',value:'1'}]
const rows=ref([]),inventoryMap=ref({}),loading=ref(false),giftVisible=ref(false),costVisible=ref(false),historyVisible=ref(false),importing=ref(false),saving=ref(false),giftFormRef=ref(),costFormRef=ref()
...
function stockClass(row){const status=inventoryMap.value[row.giftId]?.stockStatus;return status==='zero'?'stock-zero':status==='low'?'stock-low':'stock-ok'}
function stockType(row){const status=inventoryMap.value[row.giftId]?.stockStatus;return status==='zero'?'danger':status==='low'?'warning':'success'}
function stockLabel(row){const status=inventoryMap.value[row.giftId]?.stockStatus;return status==='zero'?'零库存':status==='low'?'低库存':'正常'}
async function load(){loading.value=true;try{const [giftRes,stockRes]=await Promise.all([listGifts({...query.value,...(props.embedded?{}:{includeHidden:'1'})}),listGiftInventory({status:'0'})]);rows.value=giftRes.data||[];inventoryMap.value=Object.fromEntries((stockRes.data||[]).map(item=>[item.giftId,item]))}finally{loading.value=false}}
...
.cost-change{font-size:15px;vertical-align:-1px}.cost-change.up{color:#dc2626}.cost-change.down{color:#16a34a}.stock-mini{display:flex;flex-direction:column;gap:3px;min-width:74px}.stock-mini small{margin:0;color:#909399}.stock-mini strong{font-size:15px}.stock-ok{color:#16a34a}.stock-low{color:#d97706}.stock-zero{color:#dc2626}
.gift-grid{grid-template-columns:repeat(auto-fill,minmax(360px,1fr))}.gift-card-main{grid-template-columns:minmax(82px,.65fr) minmax(90px,.72fr) minmax(150px,1.25fr);gap:10px}.stock-mini .el-tag{width:max-content}
@media(max-width:430px){.gift-grid{grid-template-columns:1fr}.gift-card-main{grid-template-columns:1fr 1fr}.history-entry{grid-column:1/-1}}
.gift-card{padding:0;overflow:hidden}.gift-card-head{padding:15px 16px 13px}.gift-title strong{font-size:16px}.status-control{display:flex;align-items:center;gap:7px;color:#909399;font-size:11px}.gift-metrics{display:grid;grid-template-columns:1fr 1fr;border-top:1px solid #eef0f2;border-bottom:1px solid #eef0f2;background:#fafbfc}.gift-metrics>div{display:flex;min-width:0;flex-direction:column;gap:5px;padding:13px 16px}.gift-metrics>div+div{border-left:1px solid #e8ebef}.gift-metrics small{color:#7a8591}.gift-metrics strong{font-size:21px}.gift-metrics em{margin-left:3px;font-size:12px;font-style:normal;font-weight:400}.stock-state{font-size:11px}.alias-line{display:grid;grid-template-columns:62px minmax(0,1fr);gap:8px;margin:0;padding:11px 16px;color:#7a8591}.alias-line span{font-size:11px}.alias-line p{overflow:hidden;margin:0;font-size:12px;text-overflow:ellipsis;white-space:nowrap}.gift-card-actions{margin:0;padding:8px 10px;border-top:1px solid #eef0f2;background:#fff}.action-spacer{flex:1}.gift-card.disabled .gift-metrics{background:#f3f4f6}
</style>
