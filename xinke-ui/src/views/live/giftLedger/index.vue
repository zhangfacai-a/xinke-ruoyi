<template>
  <div class="app-container gift-ledger">
    ...
    <section class="room-context">
      <div class="room-context-copy"><span>当前直播间</span><strong>{{ currentRoom?.roomName || '全部直播间' }}</strong></div>
      <el-select v-model="currentRoomId" clearable filterable placeholder="全部直播间（可不选）"
        @change="changeCurrentRoom"><el-option v-for="item in rooms" :key="item.roomId"
          :label="`${item.roomName} · ${item.roomCode}`" :value="item.roomId" /></el-select>
      <div class="room-people"><span>主播</span><strong>{{ currentRoomPeople.anchor || '未配置' }}</strong></div>
      <div class="room-people"><span>场控</span><strong>{{ currentRoomPeople.controller || '未配置' }}</strong></div>

      <div class="entry-mode-row">
        <el-segmented v-model="entryMode"
          :options="[{ label: '单个订单', value: 'single' }, { label: '批量订单', value: 'batch' }]" />
        <div v-if="entryMode === 'single'" class="single-entry-row">
          <el-input ref="quickOrderInput" v-model.trim="quickOrderNo" size="large" clearable maxlength="64"
            placeholder="输入平台订单号，按回车开始" @keyup.enter="openEntry(quickOrderNo)">
            <template #prefix><el-icon>
                <Tickets />
              </el-icon></template>
          </el-input>
          <el-button type="primary" size="large" :disabled="!quickOrderNo"
            @click="openEntry(quickOrderNo)">录入</el-button>
        </div>
        <div v-else class="batch-entry-row">
          <el-input v-model="batchOrderText" type="textarea" :rows="3" resize="none"
            placeholder="粘贴订单号：支持 Excel 一列、换行、逗号或空格，自动去空和去重" />
          <div class="batch-entry-side">
            ...
            <el-button type="primary" size="large"
              :disabled="!parsedBatchOrders.length || parsedBatchOrders.length > 500"
              @click="openBatchEntry">统一配置</el-button>
            </div>
        </div>
      </div>

      <el-alert v-if="entryMode === 'batch' && parsedBatchOrders.length > 500" title="单次最多录入 500 个订单，请分批处理"
    ...
    <section class="filter-bar">
      ...
      <el-date-picker v-model="dates" type="daterange" value-format="YYYY-MM-DD" start-placeholder="操作开始日"
        end-placeholder="操作结束日" @change="load" />
    ...
    <el-table v-loading="loading" :data="rows" stripe @row-dblclick="editRecord">
      ...
      <el-table-column prop="roomNameSnapshot" label="直播间" min-width="140" show-overflow-tooltip><template
          #default="{ row }">{{ row.roomNameSnapshot || '未指定' }}</template></el-table-column>
      <el-table-column label="主播 / 场控" min-width="150" show-overflow-tooltip>
        <template #default="{ row }"><span>{{ [row.anchorName, row.controllerName].filter(Boolean).join(' / ') || '-'
        }}</span></template>
      </el-table-column>
      <el-table-column prop="giftText" label="礼品" min-width="220" show-overflow-tooltip>
        <template #default="{ row }"><span :class="{ muted: !row.giftText }">{{ row.giftText ||
          statusMeta(row.processStatus).label }}</span></template>
      </el-table-column>
      <el-table-column label="到返金额" width="105" align="right"><template #default="{ row }">{{ row.refundAmount == null ?
        '-' : `¥${money(row.refundAmount)}` }}</template></el-table-column>
      <el-table-column label="服务项" min-width="150"><template #default="{ row }">
          <div class="service-tags"><el-tag v-for="item in serviceTags(row)" :key="item" size="small" type="warning"
              effect="light">{{ item }}</el-tag><span v-if="!serviceTags(row).length" class="muted">-</span></div>
        </template></el-table-column>
      <el-table-column label="成本" width="110" align="right">
        <template #default="{ row }"><strong>¥{{ money(row.giftCost) }}</strong></template>
      </el-table-column>
      <el-table-column label="状态" width="112">
        <template #default="{ row }"><el-tag :type="statusMeta(row.processStatus).type" effect="light">{{
          statusMeta(row.processStatus).label }}</el-tag></template>
      </el-table-column>
      ...
      <el-table-column v-hasPermi="['live:gift:entry']" label="操作" width="82" fixed="right">
        <template #default="{ row }"><el-button link type="primary" icon="Edit"
            @click="editRecord(row)">修改</el-button></template>
      </el-table-column>
      <template #empty><el-empty description="还没有礼品记录" /></template>
    </el-table>

    <el-drawer v-model="entryVisible" :title="entryTitle" size="min(960px, 96vw)" destroy-on-close
      @opened="focusEntryInput">
      <div class="drawer-body">
        <div v-if="activeEntryMode === 'single'" class="order-lookup">
          <el-input ref="entryOrderInput" v-model.trim="entryOrderNo" size="large" clearable maxlength="64"
            placeholder="输入订单号后按回车" @keyup.enter="findOrder">
            <template #prepend>订单号</template>
          </el-input>
          <el-button type="primary" size="large" :loading="orderLoading" :disabled="!entryOrderNo"
            @click="findOrder">确认</el-button>
        </div>
        ...
        <template v-else>
          <div class="entry-identity">
            <div>
              ...
              <el-tag v-if="activeEntryMode === 'single'" :type="isEditing ? 'warning' : 'success'" effect="light">{{
                isEditing ? '已有记录 · 修改' : '首次录入 · 新增' }}</el-tag>
            </div>
            <div><span>录入人</span><strong>{{ currentUser }}</strong></div>
            <div><span>直播间</span><strong>{{ entryForm.roomNameSnapshot || '未指定' }}</strong></div>
          </div>
          <div v-if="activeEntryMode === 'batch'" class="batch-policy">
            <div><strong>订单号已清洗</strong><span>{{ batchOrderNos.slice(0, 5).join('、') }}{{ batchOrderNos.length > 5 ? ` 等
                ${batchOrderNos.length} 个` : '' }}</span></div>
          ...
          <section class="entry-section template-section">
            <div class="section-title">
              <div><strong>快捷模板</strong><span>点一下填入整套订单信息，之后仍可修改</span></div>
            </div>
            <div class="template-chips">
              <button type="button" :class="{ active: activeTemplateId == null }" @click="useManualMode"><el-icon>
                  <EditPen />
                </el-icon>自定义</button>
              <button v-for="item in enabledTemplates" :key="item.templateId" type="button"
                :class="{ active: activeTemplateId === item.templateId }" @click="applyTemplate(item)"><el-icon>
                  <MagicStick />
                </el-icon>{{ item.templateName }}</button>
          ...
          <section class="entry-section service-section">
            <div class="section-title">
              <div><strong>订单服务信息</strong><span>空白项不计入预览和统计</span></div>
            </div>
            <div class="service-grid">
              <el-form-item label="主播"><el-select v-model="entryForm.anchorUserId" filterable clearable
                  placeholder="选择主播" @change="syncStaffSnapshot"><el-option v-for="item in anchorOptions"
                    :key="item.userId" :label="item.userName" :value="item.userId" /></el-select></el-form-item>
              <el-form-item label="场控"><el-select v-model="entryForm.controllerUserId" filterable clearable
                  placeholder="选择场控" @change="syncStaffSnapshot"><el-option v-for="item in controllerOptions"
                    :key="item.userId" :label="item.userName" :value="item.userId" /></el-select></el-form-item>
              <el-form-item label="到返金额"><el-input-number v-model="entryForm.refundAmount" :min="0" :precision="2"
                  :step="1" :value-on-clear="null" controls-position="right" /></el-form-item>
              <el-form-item label="服务标记"><el-input v-model="entryForm.serviceMark" clearable
                  placeholder="例如：重点跟进" /></el-form-item>
              <el-form-item label="到返理由" class="span-2"><el-input v-model="entryForm.refundReason" clearable
                  placeholder="填写到返原因" /></el-form-item>
              <el-form-item label="售后补偿" class="span-2"><el-input v-model="entryForm.afterSaleCompensation" clearable
                  placeholder="填写售后补偿内容" /></el-form-item>
              <el-form-item label="其他备注" class="span-4"><el-input v-model="entryForm.otherRemark" type="textarea"
                  :rows="2" maxlength="500" placeholder="补充说明" /></el-form-item>
            </div>
          ...
          <div class="gift-workspace">
            <section class="gift-catalog">
              ...
              <div class="gift-grid">
                <button v-for="item in filteredGifts" :key="item.giftId" type="button"
                  :disabled="item.currentCost == null" @click="chooseGift(item)">
            ...
            <section ref="selectedListRef" class="selected-gifts">
              ...
              <div v-for="item in (entryForm.gifts || [])" :key="item.giftId" class="selected-row">
                <div class="selected-name"><strong>{{ item.giftName }}</strong><span>¥{{ money(item.currentCost) }}/{{
                  item.unit || '件' }}</span></div>
      ...
      <template #footer>
        <div class="drawer-actions">
          ...
          <div>
            <el-button v-if="activeEntryMode === 'single'" :disabled="!order" :loading="saving"
              @click="saveEntry(false)">保存</el-button>
            <el-button type="primary" :disabled="!order" :loading="saving"
              @click="saveEntry(activeEntryMode === 'single')">{{ activeEntryMode === 'batch' ? `确认录入
              ${batchOrderNos.length} 单` : '保存并录下一单' }}</el-button>
          </div>
        </div>
      </template>
    </el-drawer>
...
.room-context {
  display: grid;
  grid-template-columns: minmax(210px, 1fr) minmax(280px, 1.4fr) minmax(120px, .6fr) minmax(120px, .6fr);
  align-items: center;
  gap: 16px;
  margin-bottom: 12px;
  padding: 13px 16px;
...
.room-context-copy span,
.room-people span {
  color: #6b7280;
  font-size: 11px
}

.room-context-copy strong,
.room-people strong {
  overflow: hidden;
...
.room-context-copy small {
  color: #909399
}

.room-context>.el-select {
  width: 100%
}
...
.single-entry-row {
  display: grid;
  grid-template-columns: minmax(280px, 680px) 88px;
...
.batch-entry-side span,
.batch-entry-side small {
  color: #6b7280
}

.batch-entry-side strong {
  color: #c2410c;
  font-size: 18px
}

.entry-workbench>.el-alert {
  margin-top: 10px
}
...
.service-grid :deep(.el-form-item) {
  margin-bottom: 12px
}

.service-grid :deep(.el-select),
.service-grid :deep(.el-input),
.service-grid :deep(.el-input-number) {
  width: 100%
}
...
.gift-catalog .section-title .el-input {
  width: 220px
}
...
.gift-grid>button:hover:not(:disabled) {
  border-color: #f26b21;
  background: #fff7ed
}

.gift-grid>button:disabled {
  cursor: not-allowed;
  opacity: .55
}
...
.selected-row {
  display: grid;
  grid-template-columns: minmax(0, 1fr) 105px 76px 30px;
  align-items: center;
  gap: 7px;
  padding: 10px 0;
